...
```bash
docker pull sameersbn/gitlab:19.0.8
```
...
- GitLab CE version 19.0.8 and later requires PostgreSQL version 17.x.
...
docker run --name gitlab -d [OPTIONS] \
    sameersbn/gitlab:19.0.8 app:sanitize
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:19.0.8 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:19.0.8 app:rake db:setup
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:19.0.8 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:19.0.8 app:rake gitlab:backup:restore BACKUP=1515629493_2020_12_06_13.0.6
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:19.0.8 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:19.0.8 app:rake gitlab:import:repos
```
...
> - As of version 19.0.8, the required PostgreSQL version is 17.x.
>
...
> **Note**
>
> Upgrading to `sameersbn/gitlab:19.0.8` from `sameersbn/gitlab:7.x.x` can cause issues. It is therefore required that you first upgrade to `sameersbn/gitlab:8.0.5-1` before upgrading to `sameersbn/gitlab:8.1.0` or higher.
...
```bash
docker run --name gitlab -d [OPTIONS] sameersbn/gitlab:19.0.8
```
